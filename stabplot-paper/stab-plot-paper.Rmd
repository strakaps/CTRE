...
```{r simulated-example, message=FALSE, fig.cap="\\label{fig:sim}Tail and scale estimates for a simulated simulated data, with Mittag-Leffler waiting times ML(0.8, 10) and unit exponentially distributed magnitudes."}
tail <- 0.7
scale <- 10
times <- cumsum(MittagLeffleR::rml(n = 1000, tail = tail, scale = scale))
magnitudes <- rexp(n = 1000)
mrp <- new_mrp(times, magnitudes)
par(mfrow=c(1,2))
plot(mrp, what = "MLtail", hline = tail)
plot(mrp, what = "MLscale", tail = tail, hline = scale)
```
...
```{r, message=FALSE, fig.cap="\\label{fig:flares}Stability plots for the tail and scale parameter of the Mittag-Leffler distribution of the Solar Flare dataset."}
flares_mrp <- new_mrp(flares$times, flares$magnitudes)
par(mfrow=c(1,2))
plot(flares_mrp, what = "MLtail", hline = 0.9)
plot(flares_mrp, what = "MLscale", tail = 0.9, hline = 0.075)
```
