...
abstract: |
  ...
  whose scale parameter increases as $p^{1/\beta}$, where 
  $p$ is the threshold crossing probability and $\beta \in (0,1)$
  ...
  methods provide useful results on simulated and real-world datasets.
...
A commonly taken assumption is that times between events are either fixed 
...
fitting data to a CTRM. The following two quantities are our main 
ingredients.
...
Figure \ref{fig:GP-simulated} shows 

```{r GPD-simulated-data, warning=FALSE, fig.cap="\\label{fig:GP-simulated}Stability plot for the shape and scale parameters of the generalized Pareto distribution. Here, the magnitudes are standard Gumbel, hence $\\xi = 0$ and $\\sigma = 1$."}
par(mfrow = c(1,2))
JJ <- mrp("getMags")
...
, one chooses a threshhold $l$ and due to Theorem 3 
...
and their inter-arrival times. Hence, the above model assumptions need, from a theoretical point of view, only apply to the not discarded data, the $k_\text{max}$-largest observations. We hence propose to produce the above three model-checking plots for the 
...
plot(mrp, what = "MLscale", tail = tail, hline = n^(1/tail))
```
