...
We introduce the CTRM, or max-renewal model, in Section 2, and 
...
The case where one considers the maximum of the first $(N(t)+1)$'s event is sometimes also analyzed and called OCTRM (Oracle Continuous Time Random Maxima). By considering that this is the same as just changing the order of waiting times and events from $W_1,J_1,W_2,J_2,..$ to $J_1,W_1,J_2,W_2,...$ it is obvious that the OCTRM has in the uncoupled case the same scaling limit as the CTRM. But in the coupled case the limit processes differ slightly (for detail see @Hees16).
...
Since $J_{\tau(\ell)}$ with $\tau(l)=\min\{k: J_k > \ell\}$ is due to the renewal proparty independent of the $\sigma$-algebra generated by
$W_1, W_2, \ldots, W_{\tau(\ell)}$, the two random variables $X(\ell) = J_{\tau(\ell)}-\ell$ and $T(\ell) = W_1 + \ldots + W_{\tau(\ell)}$ are independent. 

To make inference for our CTRM we will later extract realizations of $X(\ell)$ and $T(\ell)$ out of the data by considering different level $\ell$ above a minimim threshhold $\ell_0$ (e.g. $\ell_0=95\%$ quantile). Let $X(\ell,i)$ be the $ith$ event which exceeds the level $\ell$ for $i \geq 1$. Furthermore we denote with $T(\ell,i)$ the inter-arrival time between the $(i-1)$th and the $i$th event which exceed the level $\ell$, namely $X(\ell,i-1)$ and $X(\ell,i)$, with $T(\ell,1):=T(\ell)$. By doing this for different threshholds on the intervall $[\ell_0,x_R]$, we can construct i.i.d.\ sequences $\{T(\ell,i)\}_{i}$ and $\{X(\ell,i)\}_{i}$ for any given threshold $\ell$.
...
mrp <- new_mrp(data.frame(times, magnitudes))
plot(mrp)
```


Figure \ref{fig:thresholdedBursty} illustrates that $T(\ell, i)$ and 
$X(\ell, i)$ are directly observable in data: Event magnitudes $J_k$ are 
grey vertical lines at their arrival times $W_1 + \ldots + W_k$. 
Thresholding at a level $\ell$ (dashed line) gives a series of 
$\{X(\ell,i)\}_{i}$ (red) and the preceding times until exceedance 
$\{T(\ell, i)\}_{i}$ (distances between adjacent blue crosses).
...
Now we get
\begin{align}
\mathbf P \left(T(\ell)/b(1/p) > t \right) &= \mathbf P(M(b(1/p) t) \leq \ell) \label{eq:proofTheo}
= \mathbf P(M(b(1/p)) \le a(c) \ell^* + d(c))\\
...
\end{equation}
In \eqref{eq:proofTheo2} we used \eqref{eq:np_conv} and the fact that since $b \in RV(1/\beta)$, $b(\lambda c) / b(c) \rightarrow \lambda^{1/\beta}$ as $c \rightarrow \infty$ uniformly in $\lambda$ on each compact subset of $(0,\infty)$. The right side of \eqref{eq:proofTheo} is equal to
...
generalizes the standard Poisson process, which models the threshold
crossing times if times between events are light-tailed. 
...
We also note that a fractional Poisson process can be represented as a
...
standard Poisson process $N(t)$, and accordingly the exceedance times 
...
The above, theoretically sound approach to the estimation of the 
Mittag-Leffler parameters, needs the following adjustments for 
practicability:
...
<<<<<<< HEAD
The aim of a CTRM model is to fit the extreme observations
and their inter-arrival times. Hence, the above model assumptions need, from a 
theoretical point of view, only apply to the not discarded data, the $k_\text{max}$-largest observations. We hence propose to produce the above three model-checking plots for the 
=======
...
**discarded**. 
In the same vein, the above model assumptions need, from a 
...
>>>>>>> a89360c321a5fd7f449bba72046c2176ab307c0f
...
bitcoin_mrp <- new_mrp(bitcoin)
plot(bitcoin_mrp, p = 0.02, what = "data")
```
...
par(mfrow=c(3,2)) 
plot(bitcoin_mrp, what = "diagnostics")
```
