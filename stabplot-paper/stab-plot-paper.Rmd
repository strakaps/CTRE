...
authors: 
- name: "Katharina Hees"
  thanks: Katharina Hees was supported by the DAAD co-financed by the
...
- name: "Peter Straka"
  thanks: Corresponding author. Peter Straka was supported by the 
...
abstract: |
  ...
  given threshold $\ell$. We derive limit theorems for the distribution of
  $X(\ell)$ and $T(\ell)$ as $\ell$ approaches extreme values. 
  The standard Peaks Over Threshold inference approach in extreme value
  theory is based on the fact 
  that $X(\ell)$ is approximately generalized Pareto distributed, and 
  models the threshold crossing times as a standard Poisson process.
  We show that for waiting times with infinite mean, the threshold 
  crossing times approach a *fractional* Poisson process in the limit 
  of high thresholds. The inter-arrival times of threshold crossings
  scale with $p^{1/\beta}$, where 
  $p$ is the threshold crossing probability and $\beta \in (0,1)$
  is the tail parameter for the waiting times. We provide
...
$D(t)$ is a stable subordinator, i.e. an increasing L\'evy process with
...
\ref{magnitudes-and-extreme-value-distributions}, and assume WLOG that there exists an $n$ such that
...
A Hill plot (Figure \ref{fig:bitcoin-diag}) of the exceedance times at the 
level of the 100th 
...
plot(flares_mrp, what = "MLscale", tail = 0.85, hline = 2.5 * 1E7)
```
...
par(mfrow=c(3,2))
plot(flares_mrp, what = "diagnostics", tail = 0.85)
```
