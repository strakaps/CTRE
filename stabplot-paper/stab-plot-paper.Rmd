...
\begin{equation}
b(1/p)/b(c) = b(1/cp \cdot c) \to \left( -\log F_A(l)\right)^{-1/\beta} \text{ as } c \to \infty.\label{eq:proofTheo2}
\end{equation}
In \eqref{eq:proofTheo2} we used \eqref{eq:np_conv} and the fact that since $b \in RV(1/\beta)$, $b(\lambda c) / b(c) \rightarrow \lambda^{1/\beta}$ as $c \rightarrow \infty$ uniformly on each compact subset of $(0,\infty)$. The right side of \eqref{eq:proofTheo} is equal to
